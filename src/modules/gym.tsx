...
import { StraightMap } from "./maps/training/trainingsMaps"
...
export enum TrainingsParameter {
...
export class Gym {
    ...
    trainingsParameter: TrainingsParameter

    setHighscore: (i: number) => void
    ...
    constructor(width, height, map) {
        ...
        this.trainingsParameter = TrainingsParameter.Checkpoints
    }

    init(loadedMap) {
        const { nodes, edges } = loadedMap()
        this.nodes = nodes
        this.edges = edges
    ...
    step() {
        const evaluate = () => {
            ...
            allAgents.sort((a, b) => a[this.trainingsParameter] > b[this.trainingsParameter] ? -1 : 0)

            // const lifetime = allAgents[0].lifetime
            const parameterValue = allAgents[0][this.trainingsParameter]

            if (parameterValue > this.mostCheckpoints) {
                console.log({ parameterValue })
                //this.pretrain = false
                this.mostCheckpoints = parameterValue
                this.bestNeuralNet = allAgents[0].nn.copy()
                this.setHighscore(parameterValue)
            }
    ...
    toggleTrainigsParameter() {
        if (this.trainingsParameter === TrainingsParameter.Checkpoints) {
            this.trainingsParameter = TrainingsParameter.Lifetime
        } else {
            this.trainingsParameter = TrainingsParameter.Checkpoints
        }
        this.mostCheckpoints = 0
    }
}
